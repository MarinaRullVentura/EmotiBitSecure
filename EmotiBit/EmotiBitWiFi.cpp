...
{
	_crypto.loadKeysFromFile("/sysinfo.ini");
...
int8_t EmotiBitWiFi::readUdp(WiFiUDP &udp, String &message)
{
	...
	if (packetSize > 0)
	{
		std::vector<uint8_t> rawPacket(packetSize);
		int len = udp.read(rawPacket.data(), packetSize);

		if (len > 0)
		{
			String decrypted;
			if (_crypto.decryptAndVerifyHMAC(rawPacket, decrypted)) {
				message = decrypted;
				return SUCCESS;
			}
			else {
...
{
	return sendUdp(_advertisingCxn, message, ip, port, false);
}
...
int8_t EmotiBitWiFi::sendData(const String &message)
{
	if (_isConnected)
	{
		...
		return sendUdp(_dataCxn, message, _hostIp, _dataPort, true);
	}
	// ToDo: implement logic to determine return val
...
int8_t EmotiBitWiFi::sendUdp(WiFiUDP& udp, const String& message, const IPAddress& ip, uint16_t port, bool isData)
{
	int8_t wifiStatus = status();
	if (wifiStatus != WL_CONNECTED)
	{
	...
	while (firstIndex < message.length()) {
		...
		String subMessage = message.substring(firstIndex, lastIndex + 1);
		
		std::vector<uint8_t> encryptedMsg;
		if (isData)
		{
			if (!_crypto.encryptAndSignData(subMessage, encryptedMsg)) {
				return FAIL;
			}
		}
		else
		{
			if (!_crypto.encryptAndSignControl(subMessage, encryptedMsg)) {
				return FAIL;
			}
		}
		...
		for (uint8_t n = 0; n < _nDataSends; n++) {
			...
			udp.write(encryptedMsg.data(), encryptedMsg.size());
			udp.endPacket();
		}
