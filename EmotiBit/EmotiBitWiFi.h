...
public:
	...
	int8_t sendUdp(WiFiUDP &udp, const String &message, const IPAddress &ip, uint16_t port, bool isData);
  ...
  6	WL_DISCONNECTED	when disconnected from a network
	*/
	...
	bool loadPreSharedKey();
	bool verifyAndDecryptHMAC(const std::vector<uint8_t>& input, String& plaintextOut);
	uint8_t _psk_eb[32];
	uint8_t _psk_osc[32];	
	bool encryptAndHmac(const String& message, std::vector<uint8_t>& outEncrypted);
