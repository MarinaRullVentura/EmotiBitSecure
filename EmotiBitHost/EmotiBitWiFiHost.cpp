...
{
    bool ok = _crypto.loadKeysFromUrl("http://localhost/emotibitSecMan/get_psk.php");
    if (!ok) {
        ofLogError() << "❌ Failed to load PSKs from URL";
        return FAIL;
    }
...
//        advertisingCxn.Send(packet.c_str(), packet.length());
        vector<uint8_t> encrypted = _crypto.encryptAndSignWithOscilloscope(packet);
        advertisingCxn.Send(reinterpret_cast<const char*>(encrypted.data()), encrypted.size());
...
int8_t EmotiBitWiFiHost::processAdvertising(vector<string> &infoPackets)
{
    ...
    if (checkAdvertisingTime >= _wifiHostSettings.checkAdvertisingInterval)
    {
        ...
        if (msgSize > 0)
        {
            ...
            vector<uint8_t> raw(reinterpret_cast<uint8_t*>(udpMessage), reinterpret_cast<uint8_t*>(udpMessage) + msgSize);
            std::vector<std::string> packets;
            std::vector<uint8_t> cipher;
            std::vector<uint8_t> hmac;
            if (!_crypto.decryptAdvertisement(raw, packets, cipher, hmac)) {
                ofLogWarning() << "❌ Failed to decrypt advertisement";
                return FAIL;
            }
            ...
            for (string packet : packets)
            {
                ...
                if (dataStartChar != 0)
                {
                    if (header.typeTag.compare(EmotiBitPacket::TypeTag::HELLO_HOST) == 0)
                    {
                        ...
                        int16_t deviceIdPos = EmotiBitPacket::getPacketKeyedValue(packet, EmotiBitPacket::PayloadLabel::DEVICE_ID, emotibitDeviceId, dataStartChar);
                        if (deviceIdPos == -1) {
                            std::cout << "HELLO_HOST sense DEVICE_ID. Ignorat.";
                            continue;
                        }

                        if (!_crypto.verifyHelloHost(cipher, hmac, emotibitDeviceId)) {
                            std::cout << "HELLO_HOST HMAC o ID invàlid: " << emotibitDeviceId;
                            continue;
                        }
                    ...
                    {
                        // PONG
                        if (!_crypto.verifyConnectedPacket(cipher, hmac)) {
                            ofLogWarning() << "❌ Invalid PONG from connected EmotiBit";
                            continue;
                                    }
...
void EmotiBitWiFiHost::readUdp(ofxUDPManager &udp, string& message, string ipFilter)
{
    ...
    if (ipFilter.length() != 0 && ip != ipFilter)
    {
        message = "";
        return;
    }
    ...
    if (isConnected()) {
        if (!_crypto.decryptAndVerifyFromConnected(raw, plaintext)) {
            std::cout << "[readUDP] No s’ha pogut verificar ni desxifrar un missatge de dades provinent de l’EmotiBit actualment connectat.";
            std::cout << std::endl;
            std::cout << std::endl;
                ...
                else
                {
                    ...
                    else
                    {
                         ...
                         if (startChar == 0)
                        {
                            ...
                            {
                                // Primer paquet rebut després de connectar un EmotiBit
                                receivedDataPacketNumber = header.packetNumber;
                                firstPacketReceived = true;
                            }
                            else if (header.packetNumber == receivedDataPacketNumber)
                            {
                                // Paquet duplicat (mateix número que l’anterior) → ignorat
                                return;
                            }
                            else if ((uint16_t)(header.packetNumber - receivedDataPacketNumber) > 32768)
                            {
                                // Paquet massa antic (desfasament gran) → descartat
                                return;
                            }
                            else
                            {
                                // Paquet vàlid → actualitzem el número
                                receivedDataPacketNumber = header.packetNumber;
                            }
                        }
...
//        dataCxn.Send(packet.c_str(), packet.length());
        vector<uint8_t> encrypted = _crypto.encryptAndSignWithOscilloscope(packet);
        dataCxn.Send(reinterpret_cast<const char*>(encrypted.data()), encrypted.size());
        dataCxnMutex.unlock();
        return SUCCESS;
    }
    else
...
int8_t EmotiBitWiFiHost::connect(string deviceId)
{
    ...
    _crypto.setConnectedDevice(deviceId);
